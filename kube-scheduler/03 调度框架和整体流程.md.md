...
从上图中可以看出，在`Pod`加入调度队列以后，包含了两个`Cycle`，也就是说在整个调度的过程中，包含两个大的生命周期。在[调度器的创建和调度队列](./01 调度器的创建和调度队列.md)提到过实例启动的最外层逻辑是`Run()`方法。
